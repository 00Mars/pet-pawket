...
export function injectNavbar(callback){
  try { window.__ppDropdownDebug?.push('navbar.inject:start', { when: Date.now() }); } catch {}
  fetch('/navbar.html', { credentials: 'include' })
    .then((res) => { if (!res.ok) throw new Error(`HTTP ${res.status}`); return res.text(); })
    .then((html) => {
      let container = document.getElementById('navbar-container');
      if (!container){ container = document.createElement('div'); container.id = 'navbar-container'; document.body.appendChild(container); }
      ...
      (function normalizeIds() {
        ...
        for (const [oldId, nextId] of Object.entries(map)) {
          ...
          if (oldEl && !nextEl) {
            oldEl.id = nextId;
          } else if (oldEl && nextEl) {
            // Remove the old element to prevent duplicate/conflicting IDs
      ...
      try { callback?.(); try { window.__ppDropdownDebug?.push('navbar.inject:callback', { when: Date.now() }); } catch {} } catch (e) { console.warn('[injectNavbar] callback warn:', e); }
      ...
      toggles.forEach((t) => {
        if (!t.dataset.wiredMobileMenu) {
          t.addEventListener('click', toggleMobileMenu);
          t.dataset.wiredMobileMenu = '1';
        }
      });
    ...
    .catch((err) => {
      console.error('[injectNavbar] Injection failed:', err);
      if (err && err.stack) {
        console.error('[injectNavbar] Stack trace:', err.stack);
      }
      console.warn('[injectNavbar] Please check if /navbar.html is accessible, and verify network connectivity or server errors.');
    });
}
